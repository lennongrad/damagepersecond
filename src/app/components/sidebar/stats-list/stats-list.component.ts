...
export class StatsListComponent implements OnInit {
  ...
  getStat(stat: BaseStatTypes): string{
    return this.unitInstancesService.selectedCharacter.getStat(stat, false, false).toString();
  }

  getModifier(stat: BaseStatTypes): string{
    var base = this.unitInstancesService.selectedCharacter.getStatModifier(stat, false, false);
    if(base > 0){
